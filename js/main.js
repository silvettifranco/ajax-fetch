...
$("#reservas").append(`<form id="myForm">
                    ...
                    <input id='campo1' type="text" placeholder="Ej: Franco Silvetti">
...
/* 
-----Aplicación de AJAX-------    */           
$(".masAlquilados").prepend('<button id="boton1">CONSULTAR UNIDADES MÁS ALQUILADAS</button>');
...
$("#boton1").click(() => { 
    $.ajax({
        ...
        success: function(respuesta) {
            let contenido = "<div id='aviso3'>";
            
            for (let datos of respuesta) {
                contenido += "<div class='card m-3'>"
                contenido += "<div class='text-center text-dark bg-light'><h4>"+datos.nombre+"</h4></div>"
                contenido += "<img src='img/" + datos.imagen + "' widht='180'>";
                contenido += "<div class='text-center text-dark bg-light'><h4>"+datos.precio+" por día"+"</h4></div>"
                contenido += "<div class='text-center text-dark bg-light'><h6>"+"Propietario: "+datos.propietario+"</h6></div>"
                contenido += "</div>"
            }
